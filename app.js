...
function dayEventHTML(l, isTeacher) {
  ...
  return `<div class="dev ${statusClass}">
    ...
    ${l.status==='scheduled' ? classroomBtnHTML(l, isTeacher) : ''}
...
function openClassroom(roomName, title, sub, userName) {
  if (!roomName) return showToast('⚠️ Sala não configurada para esta aula.');
  ...
  // Small delay to ensure modal is visible before mounting Jitsi
  setTimeout(() => mountJitsi(roomName, userName), 300);
}

function mountJitsi(roomName, userName) {
  const container = document.getElementById('jitsi-container');
  container.innerHTML = '';

  // Load Jitsi External API script if not loaded yet
  if (!window.JitsiMeetExternalAPI) {
    const script = document.createElement('script');
    script.src = 'https://meet.jit.si/external_api.js';
    script.onload = () => initJitsi(roomName, userName, container);
    script.onerror = () => {
      container.innerHTML = `<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;color:#fff;gap:16px;padding:32px;text-align:center">
        ...
        <h3 style="font-size:20px">Não foi possível carregar a sala</h3>
        <p style="color:rgba(255,255,255,.6);max-width:400px">Verifique sua conexão com a internet e tente novamente.</p>
        <a href="https://meet.jit.si/${roomName}" target="_blank" rel="noopener noreferrer" style="background:#3b6ef5;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:600">Abrir em nova aba</a>
      </div>`;
    };
    document.head.appendChild(script);
  } else {
    initJitsi(roomName, userName, container);
  }
}

function initJitsi(roomName, userName, container) {
  if (_jitsiAPI) {
  ...
  const options = {
    ...
    configOverwrite: {
      ...
      toolbarButtons: [
        ...
        'videoquality', 'tileview', 'download', 'settings'
      ],
    },
    interfaceConfigOverwrite: {
      ...
      DEFAULT_LOCAL_DISPLAY_NAME:    userName || 'Participante',
      APP_NAME:                      'BeBrave',
      NATIVE_APP_NAME:               'BeBrave',
    },
    userInfo: {
      displayName: userName || (ME ? ME.name : 'Participante'),
    },
    lang: 'pt',
  };

  try {
    _jitsiAPI = new window.JitsiMeetExternalAPI('meet.jit.si', options);
    _jitsiAPI.addEventListener('readyToClose', closeClassroom);
  } catch(e) {
    console.error('Jitsi init error:', e);
    container.innerHTML = `<div style="display:flex;align-items:center;justify-content:center;height:100%;color:#fff;flex-direction:column;gap:16px">
      <div style="font-size:40px">⚠️</div>
      <p>Erro ao iniciar a sala. <a href="https://meet.jit.si/${roomName}" target="_blank" style="color:#6b96ff">Abrir em nova aba</a></p>
    </div>`;
  }
}
...
function copyClassroomLink() {
  ...
  const link = `https://meet.jit.si/${_currentRoom}`;
  navigator.clipboard.writeText(link).then(() => {
    showToast('✅ Link da sala copiado!');
    document.getElementById('copy-link-btn').textContent = '✅ Copiado!';
    setTimeout(() => { document.getElementById('copy-link-btn').textContent = '🔗 Copiar link'; }, 2000);
  }).catch(() => {
    prompt('Copie o link da sala:', link);
  });
}

// ── Classroom button HTML helpers ─────────────────────────────
function classroomBtnHTML(lesson, isTeacher) {
  if (!lesson.jitsiRoom) return '';
  const now = new Date();
  const lessonDate = new Date(lesson.date + 'T' + lesson.time);
  const diffMin = (lessonDate - now) / 60000;
  // Show button if lesson is within 30 min before or during
  const isNearby = diffMin <= 30 && diffMin > -120;
  const isSoon   = diffMin > 30 && diffMin <= 1440; // within 24h

  if (lesson.status !== 'scheduled') return '';

  if (isNearby) {
    return `<button class="btn-classroom btn-classroom-live" onclick="openClassroom('${lesson.jitsiRoom}','${escJs(lesson.subject||lesson.topic||'Aula')}','${escJs(lesson.studentName||'')} • ${lesson.time}','${escJs(ME?ME.name:'')}')">
      🎥 Entrar na Aula
    </button>`;
  }
  return `<button class="btn-classroom" onclick="openClassroom('${lesson.jitsiRoom}','${escJs(lesson.subject||lesson.topic||'Aula')}','${escJs(lesson.studentName||'')} • ${lesson.time}','${escJs(ME?ME.name:'')}')">
    🎥 Abrir Sala
  </button>`;
}
